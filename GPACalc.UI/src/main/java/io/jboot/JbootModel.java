...
public class JbootModel<M extends JbootModel<M>> extends Model<M>
{
    ...
    /**
     * 复制一个新的model     * 主要是用在 从缓存取出数据的时候，如果直接修改，在ehcache会抛异常     * 如果要对model进行修改，可以先copy一份新的，然后再修改     *     * @return
     */
    public M copy()
    {
        M m = null;
    ...
    /**
     * 在 RPC 传输的时候，通过 Controller 传入到Service     * 不同的序列化方案 可能导致 getModifyFlag 并未设置，可能造成无法保存到数据库     * 因此需要 通过这个方法 拷贝数据库对于字段，然后再进行更新或保存     *     * @return
     */
    public M copyModel()
    {
        M m = null;
    ...
    /**
     * 修复 jfinal use 可能造成的线程安全问题     *     * @param configName     * @return
     */
    @Override
    ...
    /**
     * 根据列名和值，查找1条数据     *     * @param column     * @param value     * @return
     */
    ...
    /**
     * 根据 列和值 查询1条数据     *     * @param column     * @return
     */
    ...
    /**
     * 根据 多列和值，查询1条数据     *     * @param columns     * @return
     */
    ...
    /**
     * 根据列名和值 查询一个列表     *     * @param column     * @param value     * @param count  最多查询多少条数据     * @return
     */
    ...
    /**
     * 根据 列信息 查找数据列表     *     * @param column     * @param count     * @return
     */
    ...
    /**
     * 根据列信心查询列表     *     * @param columns     * @param orderBy     * @param count     * @return
     */
    ...
    /**
     * 分页查询数据     *     * @param pageNumber     * @param pageSize     * @return
     */
    ...
    /**
     * 根据某列信息，分页查询数据     *     * @param pageNumber     * @param pageSize     * @param column     * @return
     */
    ...
    /**
     * 根据列信息，分页查询数据     *     * @param pageNumber     * @param pageSize     * @param columns     * @return
     */
    ...
    /**
     * 根据列信息，分页查询数据     *     * @param pageNumber     * @param pageSize     * @param columns     * @param orderBy     * @return
     */
    ...
    protected Table _getTable(boolean validateNull)
    {
        if (table == null)
        {
            ...
            {
                throw new RuntimeException(String.format("class %s can not mapping to database table, maybe cannot connection to database or not use correct datasource, " + "please check jboot.properties or correct config @Table(datasource=xxx) if you use multi datasource.", _getUsefulClass().getName()));
            }
        }
        return table;
    }
